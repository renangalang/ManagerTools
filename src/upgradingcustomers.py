...
def hasUpgrading(labels):
    for label in labels:
        if label == "LSF_ESCALATED":
            return True
    return False

#MAIN METHOD============================================================
with open('upgradingids.txt') as f:
...
for idset in list(chunks(ids, 25)):
    ...
    for issue in issues:
        ...
        if not hasUpgrading(issuedata.fields.labels):
            print("Adding upgrading label to %s" % issue)            
            labels = issuedata.fields.labels
            labels.append("LSF_ESCALATED")
            issue.update(fields={"labels": labels})            
        else:
            print("%s already has escalated label" % issue)
                    
    #for issue in issues:
    #    print(issue)
print("Total upgrading count: %d" % totalcount)
sys.exit(0)